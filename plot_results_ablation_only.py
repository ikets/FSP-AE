...
def plot_ae_itd_uniform(database_info, itd_dict, itd_dict_gt, database_name, save_path, labels):
    ...
    for m, method in enumerate(methods):
        if m in [1, 2, 3]:
            ...
            plt.errorbar(num_mes_pos_list_actual, np.array(values[method]["mean"]) * 1e6, yerr=np.array(values[method]["std"]) * 1e6, capsize=5, fmt=format[method], label=method.replace("-", "\u2212"), mec="black", ms=10, color=cycle[m])

    plt.ylabel(r"$\mathrm{AE}^{(\mathrm{ITD})}$" + " ($\mu$s)", fontsize=fs + 2)
    plt.xlabel('Number of measurement positions ' + r'$B^{(\mathrm{m})}$', fontsize=fs + 2)
    plt.legend(bbox_to_anchor=(-0.15, 1.12), loc='upper left', borderaxespad=0, fontsize=fs, fancybox=False, frameon=False, ncol=3, labelspacing=0.1)
    plt.grid()
    plt.xscale("log")
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    plt.savefig(save_path, dpi=400, bbox_inches="tight")
...
def plot_lsd_plane(hrtf_mag_dict, hrtf_mag_dict_gt, database_name, save_path, labels):
    ...
    for m, method in enumerate(methods):
        ...
        for suffix in suffix_list:
            ...
            values[method]["mean"].append(th.mean(lsd))
            values[method]["std"].append(th.std(lsd))
        m_color = m if m == 0 else m + 3
        plt.bar(x_list + dh * (m - 3 / 2), values[method]["mean"], yerr=values[method]["std"], width=dh, label=method.replace("-", "\u2212"), zorder=2, hatch=patterns[m], ec="0.3", lw=1, capsize=6 / 0.2 * dh, ecolor="0", color=cycle[m_color])
    plt.grid(axis='y')
    plt.legend(bbox_to_anchor=(0., 1.1), loc='upper left', borderaxespad=0, fancybox=False, frameon=False, ncol=3)
    plt.ylabel("LSD (dB)")
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    plt.savefig(save_path, dpi=400, bbox_inches="tight")
    print(f"Saved to {save_path}.")
