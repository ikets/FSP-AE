...
def sph_wavefunc_expansion(hrtf, freq, mes_pos_sph, tar_pos_sph, reg_param=1e-6, coeff=2.0, sound_speed=343.18, r_shoulder=0.45):
    ...
    N_o_max = th.ceil(th.sqrt(th.tensor(mes_pos_sph.shape[0] * coeff))) - 1
...
def real_sph_harm_expansion(input, mes_pos_sph, tar_pos_sph, input_type="hrtf_mag", reg_param=1e-3, coeff=2.0):
    ...
    Returns:
        output: one of
            ...
            - itd_pred:      (B_t, S)
    '''
    n_vec, m_vec = sph_harm_nmvec(int(th.ceil(th.sqrt(th.tensor(mes_pos_sph.shape[0] * coeff))) - 1))
    gamma = reg_matrix("duraiswami", n_vec=n_vec)

    sph_harm_mes = sph_harm(m_vec[None, :], n_vec[None, :], mes_pos_sph[:, 1, None], mes_pos_sph[:, 2, None])  # B_m, (N+1)^2
    sph_harm_mes = th.real(sph_harm_mes) * (m_vec >= 0)[None, :] - th.imag(sph_harm_mes) * (m_vec < 0)[None, :]
    sph_harm_mes = sph_harm_mes.to(input.dtype)
...
def spatial_principal_component_analysis(hrtf_mag, tar_pos_cart, mes_idx, spc_mat, mean_vec, reg_param=1e-3, coeff=2.0):
    '''
    Args:
        input: hrtf_mag (B_m, 2, L, S)
        mes_pos_cart: (B_m, 3)
        tar_pos_cart: (B_t, 3)
        mes_idx:      list of len B_m
        spc_mat:     (B_t, B_t)
        mean_vec:    (B_t)
    ...
    Returns:
        hrtf_mag_pred: (B_t, 2, L, S)
    '''
    N = int(th.pow(th.ceil(th.sqrt(th.tensor(len(mes_idx) * coeff))), 2))  # num of SPCs
    ...
    # left
    input_l_cen = hrtf_mag[:, 0, :, :] - mean_vec[mes_idx, None, None]  # (B_m, L, S)
...
def woodworth(itd, mes_pos_cart, mes_pos_sph, tar_pos_cart, tar_pos_sph):
    ...
    alpha_mes = y_on_r_mes + th.asin(y_on_r_mes)  # (B_m)
    alpha_mes = alpha_mes.unsqueeze(-1).tile(1, itd.shape[-1])  # (B_m, S)

    y_on_r_tar = tar_pos_cart[:, 1] / tar_pos_sph[:, 0]
    alpha_tar = y_on_r_tar + th.asin(y_on_r_tar)  # (B_t)
    alpha_tar = alpha_tar.unsqueeze(-1).tile(1, itd.shape[-1])  # (B_t, S)
...
def calculate_spc_mat_and_mean_vec(hrtf_mag, pos_cart):
    '''
    Args:
        input: hrtf_mag: (B_t, 2, L, S)
        pos_cart: (B_t, 3)
    ...
    Returns:
        ...
        mean_vec: (B_t)
    '''
    pos_cart_flip = pos_cart * th.tensor([1.0, -1.0, 1.0])[None, :]
    kdt = KDTree(pos_cart_flip.cpu())
    _, idx_flip = kdt.query(pos_cart)

    input_lrcat = th.cat((hrtf_mag[:, 0, :, :], hrtf_mag[idx_flip, 1, :, :]), dim=-1)  # (B_t, L, 2S)
